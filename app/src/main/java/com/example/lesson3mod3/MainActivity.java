package com.example.lesson3mod3;
...
public class MainActivity extends AppCompatActivity implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
        ...
        try {
            switch (view.getId()) {
                ...
                case R.id.btnRAW:
                    ...
                    toast.show(); // демонстрация тоста на экране
                    mediaPlayer = MediaPlayer.create(this, R.raw.app_src_main_res_raw_flight_of_the_bumblebee); // создание дорожки с имеющимся аудио-файлом
    ...
    //переопределение метода обработки нажатия аппаратных клавиш смартфона

    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        //переопределение событий нажатия на кнопки громче,тише и назад
        switch (keyCode) {
            case KeyEvent.KEYCODE_VOLUME_UP://нажатие кнопки громче
                Toast.makeText(this,"Нажата кнопка громче",Toast.LENGTH_SHORT).show();
                return true;
            case KeyEvent.KEYCODE_VOLUME_DOWN://нажатие кнопки тише
                Toast.makeText(this,"Нажата кнопка тише",Toast.LENGTH_SHORT).show();
                return true;
            case KeyEvent.KEYCODE_BACK://нажатие кнопки назад
